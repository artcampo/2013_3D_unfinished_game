...
static const int gNumStoredShaders = 6; /*!< number of different shaders */
...
const static int ShaderClassMaxResources     = 6;
...
public:
  ...
  void Init         ( ShaderClassData *aData );
  bool Compile      ( );

  void SetResource  ( std::string* aName, ID3D10ShaderResourceView* aDesc );
  void SetMatrix    ( std::string* aName, const D3DXMATRIX* aMatrix );
