...
#include "ObjEditor/MeshVisualizationModels.hpp"
...
DeferredMeshModel* MeshComponent::convertToDeferred( ID3D10Device* aDev, D3DXVECTOR3 aCamPos , ColourMapping* aColours )
{
  DeferredMeshModel* meshDx;
  
  std::vector<VertPNT> triangles;
  for ( int i = 0; i < nFaces(); ++i )
  {
    Face f = face(i);
    ...
    D3DXVECTOR3 tangent = DxMath::unitVector( vertex( f.vId( t1.v1 )).pos,
                                              vertex( f.vId( t1.v2 )).pos );
    ...
    for (int k = 0; k < f.nDecompositionTriangles() ; ++k)
      ...
      D3DXVECTOR3 p2 = vertex( f.vId( triangle.v2 )).pos;
      D3DXVECTOR3 p3 = vertex( f.vId( triangle.v3 )).pos;
      D3DXVECTOR3 normal  = DxMath::normalOfTriangle( p1,p2,p3 );

      v.nor = normal;
      v.tan = tangent;

      v.pos = p1; triangles.push_back( v );
      v.pos = p2; triangles.push_back( v );
      v.pos = p3; triangles.push_back( v );
      }

  }

  meshDx = new DeferredMeshModel( triangles,
                                  D3DXCOLOR(1,0,0,1), aDev );
  return meshDx;
}
